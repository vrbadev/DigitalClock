...
public class CommandMaterial extends MaterialCommand implements ICommand {
	...
	public void process(DigitalClock main, Player player, String[] args) {
		...
		if(super.isUsableNumber(args[2])) {
			...
			if(super.isSolid(m)) {
				player.sendMessage(ChatColor.DARK_GREEN + DigitalClock.getMessagePrefix() + ChatColor.GREEN + " Your clock '" + args[1] + "' changed material from " + oldmat + " to "+ clock.changeMaterial(m.getId(), 0).name().toLowerCase().replace("_", " "));
			} else {
			...
			if(args[2].contains(":")) {
				...
				if(super.isUsableNumber(matdata[1])) {
					if(super.isUsableNumber(matdata[0])) {
						...
						if(super.isSolid(m)) {
							player.sendMessage(ChatColor.DARK_GREEN + DigitalClock.getMessagePrefix() + ChatColor.GREEN + " Your clock '" + args[1] + "' changed material from " + oldmat + " to "+ clock.changeMaterial(m.getId(), Integer.parseInt(matdata[1])).name().toLowerCase().replace("_", " "));
						} else {
							player.sendMessage(ChatColor.DARK_RED + DigitalClock.getMessagePrefix() + ChatColor.RED + " You can't use "+ m.name().toLowerCase().replace("_", " ") +" as a material, because it is not a block (cuboid shape)!");	
						}
					} else {
