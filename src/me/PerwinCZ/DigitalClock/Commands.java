				...
				if(sender instanceof Player) {
					...
					if(args[0].equals("create")) { // 1
						if(args.length != 2) {
							...
							player.sendMessage(ChatColor.RED + "[DigitalClock] You aren't allowed to use Main.INSTANCE command!");
						} else if(Main.INSTANCE.enableBuildUsers.containsKey(player)) {
						...
						} else if(Main.INSTANCE.getConfig().getKeys(false).contains(args[1])) {
							player.sendMessage(ChatColor.RED + "[DigitalClock] Clock with Main.INSTANCE name already exists!");
						} else {
							int count = 0;
							if(Main.INSTANCE.usersClock.get(player.getName()) != null) {
								count = Main.INSTANCE.usersClock.get(player.getName());
							}
							...
							if(limitReached == false) {
						    	Main.INSTANCE.enableBuildUsers.put(player, args[1]);
						...
						if(args.length != 2) {
							...
							player.sendMessage(ChatColor.RED + "[DigitalClock] Clock '" + args[1] + "' not found!");
						} else if(Main.INSTANCE.enableMoveUsers.containsKey(player)) {
							...
							Main.INSTANCE.enableMoveUsers.put(player, args[1]);
						...
						if(args.length != 2) {
							...
							player.sendMessage(ChatColor.RED + "[DigitalClock] Clock '" + args[1] + "' not found!");
						} else if(!Main.INSTANCE.clockTasks.containsKey(args[1])) {
							player.sendMessage(ChatColor.RED + "[DigitalClock] Clock '"+ args[1] +"' is already stopped!");
						} else {	
							Main.INSTANCE.getServer().getScheduler().cancelTask(Main.INSTANCE.clockTasks.get(args[1]));
							player.sendMessage(ChatColor.GREEN + "[DigitalClock] You have successfully stopped clock '" + args[1] + "' (task number " + Main.INSTANCE.clockTasks.get(args[1]) + ").");
							Main.INSTANCE.clockTasks.remove(args[1]);
						}
						...
						if(args.length != 2) {
							...
							Main.INSTANCE.runClock(args[1]);
							player.sendMessage(ChatColor.GREEN + "[DigitalClock] Clock '" + args[1] + "' is now running under task number " + Main.INSTANCE.clockTasks.get(args[1]) + ".");
						}
						...
						if(args.length != 3) {
							...
							if(Main.INSTANCE.clockTasks.containsKey(args[1])) {
								Main.INSTANCE.getServer().getScheduler().cancelTask(Main.INSTANCE.clockTasks.get(args[1]));
								Main.INSTANCE.clockTasks.remove(args[1]);
							}
							...
							player.sendMessage(ChatColor.GREEN + "[DigitalClock] You have successfully set countdown time on clock '" + args[1] + "' to "+ hours +":"+ minutes +":"+ seconds +". Main.INSTANCE clock is now stopped, run it by command '/digitalclock runclock <name>'.");
						}
						...
						if(args.length != 2) {
							...
							Generator.generatingSequence(clock, hours, minutes, seconds);
							player.sendMessage(ChatColor.GREEN + "[DigitalClock] You have successfully disabled countdown mode on clock '" + args[1] + "'. Main.INSTANCE clock is now stopped, run it by command '/digitalclock runclock <name>'.");
						}
						...
						if(args.length != 1) {
							...
							String list = "";
							int i = 0;
							if(Main.INSTANCE.clocks.size() != 0) {
								for(String name : Main.INSTANCE.clocks) {
									Clock clock = Clock.loadClockByClockName(name);
									list += clock.getName();
									if(i != Main.INSTANCE.clocks.size()-1) {
										list += ", ";
									}
									i++;
								}
					...
					} else if(args[0].equals("help") || args[0].equals("?")) { // 19
						if(args.length != 1) {
			    		...
			    		player.sendMessage(ChatColor.RED + "[DigitalClock] Main.INSTANCE argument doesn't exist. Show '/digitalclock help' for more info.");
			    	}
        		} else {
    				sender.sendMessage(ChatColor.RED + "[DigitalClock] Main.INSTANCE command can be executed only from the game!");
        		}
			} else {
				sender.sendMessage(ChatColor.GREEN + "---- DigitalClock v"+ Main.INSTANCE.getDescription().getVersion() +" ----\nAuthor: PerwinCZ\nRun command '/digitalclock help' in game for commands list.");
			}
			return true;
		} 
		return false;
