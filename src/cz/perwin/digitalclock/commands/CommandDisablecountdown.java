...
public class CommandDisablecountdown {
	public CommandDisablecountdown(CommandInfo info) {
		if(info.getArgs().length != 2) {
			info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Correct usage: '/"+ info.getUsedcmd() + " disablecountdown <name>'");
			...
			info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Clock '" + info.getArgs()[1] + "' hasn't enabled countdown mode!");
		} else {
			...
			info.getPlayer().sendMessage(ChatColor.DARK_GREEN + info.getMain().getMessagePrefix() + ChatColor.GREEN + " You have successfully disabled countdown mode on clock '" + info.getArgs()[1] + "'. Main.i clock is now stopped, run it by command '/"+ info.getUsedcmd() + " runclock <name>'.");
		}
	}
}
