...
public class CommandToggleseconds implements ICommand {
	...
	public String reactBadArgsSize(String usedCmd) {
		return ChatColor.DARK_RED + DigitalClock.getMessagePrefix() + ChatColor.RED + " Correct usage: '/" + usedCmd + " toggleseconds <name>'";
	}
	...
	public void process(DigitalClock main, Player player, String[] args) {
		...
		if(clock.shouldShowSeconds()) {
			...
			player.sendMessage(ChatColor.DARK_GREEN + DigitalClock.getMessagePrefix() + ChatColor.GREEN + " You have successfully hidden seconds on clock '" + args[1] + "'.");
		} else {
			clock.setShowingSeconds(true);
			player.sendMessage(ChatColor.DARK_GREEN + DigitalClock.getMessagePrefix() + ChatColor.GREEN + " You have successfully shown seconds on clock '" + args[1] + "'.");
		}
	}
}
