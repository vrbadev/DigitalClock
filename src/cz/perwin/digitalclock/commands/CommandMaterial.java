...
public class CommandMaterial extends MaterialCommand {
	...
	public CommandMaterial(CommandInfo info) {
		if(info.getArgs().length != 3) {
			info.getPlayer().sendMessage(ChatColor.RED + "[DigitalClock] Correct usage: '/"+ info.getUsedcmd() + " material <name> <material id:data>'");
		} else if(!info.getPlayer().hasPermission("digitalclock.material") && !info.getPlayer().isOp()) {
			...
			if(super.isUsableNumber(info.getArgs()[2])) {
				info.getPlayer().sendMessage(ChatColor.GREEN + "[DigitalClock] Your clock '" + info.getArgs()[1] + "' changed material from " + oldmat + " to "+ clock.changeMaterial(Integer.parseInt(info.getArgs()[2]), 0).name().toLowerCase().replace("_", " "));
			} else {
				if(info.getArgs()[2].contains(":")) {
					...
					if(super.isUsableNumber(matdata[1])) {
						if(super.isUsableNumber(matdata[0])) {
							info.getPlayer().sendMessage(ChatColor.GREEN + "[DigitalClock] Your clock '" + info.getArgs()[1] + "' changed material from " + oldmat + " to "+ clock.changeMaterial(Integer.parseInt(matdata[0]), Integer.parseInt(matdata[1])).name().toLowerCase().replace("_", " "));
						} else {
							try {
								Material m = Material.valueOf(matdata[0].toUpperCase());
								if(m != null) {
									info.getPlayer().sendMessage(ChatColor.GREEN + "[DigitalClock] Your clock '" + info.getArgs()[1] + "' changed material from " + oldmat + " to "+ clock.changeMaterial(m.getId(), Integer.parseInt(matdata[1])).name().toLowerCase().replace("_", " "));
								} else {
					...
					try {
						...
						if(m != null) {
							info.getPlayer().sendMessage(ChatColor.GREEN + "[DigitalClock] Your clock '" + info.getArgs()[1] + "' changed material from " + oldmat + " to "+ clock.changeMaterial(m.getId(), 0).name().toLowerCase().replace("_", " "));
						} else {
