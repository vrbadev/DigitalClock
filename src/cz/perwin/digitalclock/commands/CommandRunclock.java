...
public class CommandRunclock {
	public CommandRunclock(CommandInfo info) {
		if(info.getArgs().length != 2) {
			info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Correct usage: '/"+ info.getUsedcmd() + " runclock <name>'");
			...
			info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Clock '"+ info.getArgs()[1] +"' is already running!");
		} else {	
			info.getMain().run(info.getArgs()[1]);
			info.getPlayer().sendMessage(ChatColor.DARK_GREEN + info.getMain().getMessagePrefix() + ChatColor.GREEN + " Clock '" + info.getArgs()[1] + "' is now running under task number " + info.getMain().getClockTasks().get(info.getArgs()[1]) + ".");
		}
	}
}
