...
public class CommandAddingminutes {
	public CommandAddingminutes(CommandInfo info) {
		if(info.getArgs().length != 3) {
			info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Correct usage: '/"+ info.getUsedcmd() + " addingminutes <name> <minutes>'");
			...
			if(mins >= 0 && mins < 1440) {
				...
				info.getPlayer().sendMessage(ChatColor.DARK_GREEN + info.getMain().getMessagePrefix() + ChatColor.GREEN + " Clock '" + info.getArgs()[1] + "' now will add " + info.getArgs()[2] + " minutes.");
			} else {
				info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Minutes can be only integer between 0 and 1439, not " + info.getArgs()[2] + "!");
			}
		}	
	}
}
