...
public class CommandToggleblinking {
	public CommandToggleblinking(CommandInfo info) {
		if(info.getArgs().length != 2) {
			info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Correct usage: '/"+ info.getUsedcmd() + " toggleblinking <name>'");
			...
			if(clock.isBlinking()) {
				...
				info.getPlayer().sendMessage(ChatColor.DARK_GREEN + info.getMain().getMessagePrefix() + ChatColor.GREEN + " You have successfully stopped blinking of colon on clock '" + info.getArgs()[1] + "'.");
			} else {
				clock.setBlinking(true);
				info.getPlayer().sendMessage(ChatColor.DARK_GREEN + info.getMain().getMessagePrefix() + ChatColor.GREEN + " You have successfully started blinking of colon on clock '" + info.getArgs()[1] + "'.");
			}
		}
	}
}
