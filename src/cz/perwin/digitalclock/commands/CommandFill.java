...
public class CommandFill extends MaterialCommand {
	...
	public CommandFill(CommandInfo info) {
		if(info.getArgs().length != 3) {
			info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " Correct usage: '/"+ info.getUsedcmd() + " fill <name> <material id:data>'");
			...
			if(super.isUsableNumber(info.getArgs()[2])) {
				...
				if(super.isSolid(m)) {
					info.getPlayer().sendMessage(ChatColor.DARK_GREEN + info.getMain().getMessagePrefix() + ChatColor.GREEN + " Your clock '" + info.getArgs()[1] + "' changed filling material from " + oldmat + " to "+ clock.setFillingMaterial(m.getId(), 0).name().toLowerCase().replace("_", " "));
				} else {
				...
				if(info.getArgs()[2].contains(":")) {
					...
					if(super.isUsableNumber(matdata[1])) {
						if(super.isUsableNumber(matdata[0])) {
							...
							if(super.isSolid(m)) {
								info.getPlayer().sendMessage(ChatColor.DARK_GREEN + info.getMain().getMessagePrefix() + ChatColor.GREEN + " Your clock '" + info.getArgs()[1] + "' changed filling material from " + oldmat + " to "+ clock.setFillingMaterial(m.getId(), Integer.parseInt(matdata[1])).name().toLowerCase().replace("_", " "));
							} else {
								info.getPlayer().sendMessage(ChatColor.DARK_RED + info.getMain().getMessagePrefix() + ChatColor.RED + " You can't use "+ m.name().toLowerCase().replace("_", " ") +" as a filling material, because it is not a block (cuboid shape)!");	
							}
						} else {
							try {
